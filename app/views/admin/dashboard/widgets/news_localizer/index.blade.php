...
@section('form')
		...
		<form action="{{ route('admin.dashboard.widget.update', $id) }}" class='form-horizontal' method='post'>
			...
			<legend>@lang('widget/news_localizer/index.news-categories')</legend>
				...
				@if(Language::where('deleted_at', '=', null)->first() !== null)
					...
					list of categories which belong to this language. --}}
						@foreach (Language::where('deleted_at', '=', null)->get() as $language)
							...
							@if (Category::where('language_id', '=', $language->id)->whereNull('deleted_at')->first() !== null)
